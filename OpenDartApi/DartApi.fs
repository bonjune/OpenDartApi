...
type DartApi(crtfc_key: string) =
    ...
    member _.Request(url, ?queryParams) =
        Http.Request(
            url,
            query = [ "crtfc_key", crtfc_key; yield! queryParams |> Option.defaultValue [] ],
    ...
    member this.CorpCodeZipBytes(?save, ?unzip) =
        ...
        | true, false -> File.WriteAllBytes("./CORPCODE.zip", bytes)
        | _ -> ()
    ...
    member this.기업개황(corp_code) =
        ...
        let res = this.Request(url, [ nameof corp_code, corp_code ])

        match res.Body with
        | Text t -> t |> Decode.fromString ``기업개황 Response``.Decoder
        | _ -> failwith "no binary response expected"
    ...
    member this.증자감자현황(corp_code, bsns_year, reprt_code: ReportCode) =
        ...
        match res.Body with
        | Text t -> t |> Decode.fromString ``증자감자현황 Response``.Decoder
        | _ -> failwith "no binary response expected"
    ...
    member this.배당관련사항(corp_code, bsns_year, reprt_code: ReportCode) =
        ...
        match res.Body with
        | Text t -> t |> Decode.fromString ``배당관련사항 Response``.Decoder
        | _ -> failwith "no binary response expected"
    ...
    member this.``자기주식 취득 및 처분 현황``(corp_code, bsns_year, reprt_code: ReportCode) =
        ...
        match res.Body with
        | Text t -> t |> Decode.fromString ``자기주식 취득 및 처분 현황 Response``.Decoder
        | _ -> failwith "no binary response expected"
    ...
    member this.``최대주주 현황``(corp_code, bsns_year, reprt_code: ReportCode) =
        ...
        match res.Body with
        | Text t -> t |> Decode.fromString ``최대주주 현황 Response``.Decoder
        | _ -> failwith "no binary response expected"
    ...
    member this.``최대주주 변동 현황``(corp_code, bsns_year, reprt_code: ReportCode) =
        ...
        match res.Body with
        | Text t -> t |> Decode.fromString ``최대주주 변동 현황 Response``.Decoder
        | _ -> failwith "no binary response expected"
    ...
    member this.``소액주주 현황``(corp_code, bsns_year, reprt_code: ReportCode) =
        ...
        match res.Body with
        | Text t -> t |> Decode.fromString ``소액주주 현황 Response``.Decoder
        | _ -> failwith "no binary response expected"
    ...
    member this.``임원 현황``(corp_code, bsns_year, reprt_code: ReportCode) =
        ...
        match res.Body with
        | Text t -> t |> Decode.fromString ``임원현황 Response``.Decoder
        | _ -> failwith "no binary response expected"
